# Hi, I'm Jedediah (JAGENTIC) 👋

**AI/ML Engineer | Full-Stack Developer | Building Privacy-First Tools**

I'm transitioning into AI/ML engineering with a focus on building intelligent automation, content platforms, and privacy-conscious developer tools. I leverage AI augmentation in my workflow and build production-ready applications across web, mobile, and data pipelines.

## 🎯 Current Focus

- **AI-Powered Content Platforms**: Quiz game shows with automated curation and real-time features
- **Privacy-First Tools**: Local-first data storage, client-side processing, zero-tracking analytics
- **iOS Development**: SwiftUI apps with AI integration and real-time audio/video features
- **Data Pipelines**: Python-based automation for content ingestion, transformation, and delivery

## 🛠️ Tech Stack

**Languages:** Python, TypeScript, Swift, JavaScript
**Frontend:** React 18, Vite, Tailwind CSS v4, SwiftUI
**Backend/Data:** Node.js, SQLite, IndexedDB (Dexie.js)
**AI/ML:** OpenAI API (GPT-4o), prompt engineering, agent orchestration
**DevOps:** GitHub Actions, fastlane, Xcode Cloud
**Tools:** VS Code, Xcode, Claude Code, git

## 🚀 Featured Project

### [JAGENTIC Quiz Show](https://jagentic.net)
Interactive millionaire-style quiz platform with AI-curated questions and real-time tech news.

- **Tech:** Vanilla JS, GitHub Pages, OpenAI API integration
- **Features:** Daily automated quiz curation, real-time news ticker, privacy-first local storage
- **Automation:** GitHub Actions for content delivery, OpenAI agents for question generation

[View Repository →](https://github.com/Jagentic/jagentic-site)

## 📚 Background

- **Post-Grad Certificate**: AI/ML (UT Austin / Great Learning)
- **Harvard CS50P**: Python Programming
- **Professional Transition**: Moving from prior career into software engineering with AI specialization
- **Development Style**: AI-augmented workflows, reproducible pipelines, security-first architecture

## 🔗 Connect

- **Website**: [jagentic.net](https://jagentic.net)
- **LinkedIn**: [linkedin.com/in/jagentic](https://linkedin.com/in/jagentic)
- **Email**: jagentic@gmail.com

## 💡 Philosophy

> "Aggravations ∈ Opportunity"

I believe friction points are feature requests in disguise. My approach combines cutting-edge AI tools with battle-tested engineering practices to build systems that solve real problems while respecting user privacy and data sovereignty.

---

🔨 Currently building in public and documenting my transition into AI/ML engineering.
💬 Open to collaboration on privacy-first tools, iOS development, and AI agent systems.
